...
def visualize_results(X, targets, models, metrics, scaler):
    ...
    for i, col in enumerate(target_cols):
        for step in range(1, max_step+1):
            ...
            ax = axes[i, j]
            ax.plot(X.index[sample_idx:], actuals.iloc[sample_idx:], label='Actual', color='blue')
            ax.plot(X.index[sample_idx:], predictions[sample_idx:], label='Predicted', color='red', linestyle='--')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'forecast_visualization.png'))

def visualize_future_predictions(X_test, df_raw, models, metrics, scaler, target_cols, future_steps, num_samples=5):
    ...
    valid_indices = [idx for idx in test_indices if 
                    idx + pd.Timedelta(minutes=10*future_steps) <= df_raw.index.max()]
        ...
        for j, col in enumerate(target_cols):
            ...
            for step in range(1, LOOKBACK_STEPS + 1):
                ...
                if past_time in df_raw.index:
                    past_times.insert(0, past_time)  # Insert at beginning to maintain chronological order
            ...
            plt.text(start_idx, plt.ylim()[0], 'Prediction start', rotation=90, verticalalignment='bottom')
            
            # Add RMSE to the title instead of error bars
            avg_rmse = sum(pred_errors)/len(pred_errors)
            plt.title(f'Sample {i+1}: {col} prediction using {LOOKBACK_STEPS} past readings (Avg RMSE: {avg_rmse:.2f})')
            plt.xlabel('Time')
            plt.ylabel(col)
            ...
            plt.savefig(os.path.join(OUTPUT_DIR, f'future_pred_sample{i+1}_{col}.png'))
            plt.close()
...
def run():
    ...
    X, targets, feature_cols = split_features_targets(df_processed, TARGET_COLUMNS, FUTURE_STEPS)
    ...
    visualize_future_predictions(X_test, df_raw, models, metrics, scaler, TARGET_COLUMNS, FUTURE_STEPS, NUM_SAMPLES)
